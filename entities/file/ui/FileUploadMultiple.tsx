...
const FileUploadMultiple: React.FC<FileUploadMultipleProps> = ({ 
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // 파일 유효성 검사
    const validation = FILE_VALIDATION.validateFiles(Array.from(selectedFiles));
    ...
    for (let i = 0; i < selectedFiles.length; i++) {
      ...
      try {
        const { uploadUrl, key } = await fileApi.getUploadUrl(file, folder);
  ...
  const handleFileDelete = async (index: number) => {
    ...
    try {
      await fileApi.deleteFile(fileToDelete.url);
