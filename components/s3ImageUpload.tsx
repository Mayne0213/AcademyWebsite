...
const S3ImageUpload: React.FC<S3ImageUploadProps> = ({
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const apiUrl = folder 
        ? `/api/upload-url?fileType=${encodeURIComponent(selectedFile.type)}&folder=${encodeURIComponent(folder)}`
        : `/api/upload-url?fileType=${encodeURIComponent(selectedFile.type)}`;
  ...
  const handleImageDelete = async () => {
    ...
    try {
      // S3에서 실제 파일 삭제
      const deleteResponse = await fetch(`/api/delete-file?fileUrl=${encodeURIComponent(currentImageUrl)}`, {
        method: 'DELETE',
      });
