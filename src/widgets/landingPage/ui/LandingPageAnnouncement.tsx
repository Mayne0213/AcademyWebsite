...
import IconWithCircle from "@/src/shared/ui/iconWithCircle";
...
const LandingPageAnnouncement = () => {
  const { isLoading, announcements } = useAnnouncementStore();
  const { readAnnouncements } = useAnnouncementFeatureStore();

  useEffect(() => {
    // 홈페이지용 최신 공지사항 로드 (ANNOUNCEMENT_LIMIT개만)
    readAnnouncements(1, ANNOUNCEMENT_LIMIT, false);
  }, [readAnnouncements]);

  const renderContent = () => {
    if (isLoading) return <AnnouncementSkeleton />;
    if (announcements && announcements.length > 0) return <AnnouncementList announcements={announcements} />;
    return <EmptyState />;
  };
...
const AnnouncementList = ({ announcements }: { announcements: { announcementTitle: string; updatedAt: Date }[] }) => (
  <ul>
      ...
      return (
        <li key={index} className={`w-full flex justify-between py-3 font-MaruBuri-Light hover:font-MaruBuri-SemiBold hover:cursor-pointer ${STYLES.itemTextSize} ${index === 0 ? "border-y-2" : "border-b-2"}`}>
          <article className="flex justify-between items-center w-full">
            ...
            <time dateTime={updatedAt.toISOString()} className="text-gray-500">
...
export default LandingPageAnnouncement; 