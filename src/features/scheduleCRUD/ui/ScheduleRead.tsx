...
export const ScheduleRead = ({ selectedDate }: { selectedDate: Date | null }) => {
  ...
  const { handleDeleteSchedule } = useScheduleCRUDFeatureStore();
  ...
  return (
    <div className="w-full tablet:w-2/5 p-3 smalltablet:p-4 tablet:p-6 overflow-y-auto h-full">
      <div className="flex justify-between items-center mb-3 smalltablet:mb-4 tablet:mb-6">
        ...
        {selectedDate && (
          <button onClick={() => setShowAddSchedule(true)} className="bg-blue-600 text-white rounded hover:bg-blue-700 transition-colors px-2 py-1 smalltablet:px-3 smalltablet:py-1.5 tablet:px-4 tablet:py-2 text-xs smalltablet:text-sm tablet:text-base">
            스케줄 추가
          </button>
        )}
      </div>
